...
public class AcWheelDrive {
    ...
    private AcMotorEncoder leftFront;
    private AcMotorEncoder leftRear;
    private AcMotorEncoder rightFront;
    private AcMotorEncoder rightRear;
    ...
    public AcWheelDrive() {
        leftFront = new AcMotorEncoder(FrontLeftMotorName);
        leftRear = new AcMotorEncoder(BackLeftMotorName);
        rightFront = new AcMotorEncoder(FrontRightMotorName);
        rightRear = new AcMotorEncoder(BackRightMotorName);
    }
    ...
    public void linkedDrive(float power) {
        ...
        if (560 <= sideDif) {
            sideDif = -1 * (1120 - sideDif);
        }//Yay!


//sides:adjustment = (float) (.05 * Math.pow(power, 3.0000) + .2075 * power);
        /*this*/
        adjustment = (float) (.07 * Math.pow(power, 3.0000) + .4000 * power); //Super aggressive
